...
namespace Windows7.DesktopIntegration
{
    ...
    internal sealed class JumpListDestinations
    {
        ...
        public IEnumerable<string> Categories
        {
            get
            {
                foreach (var d in _categorizedDestinations.Keys)
                {
                    foreach(var k in _categorizedDestinations[d])
                        yield return k.Category;
                }
                throw new Exception();
            }
        }
            ...
            string category)
        {
            foreach (var k in _categorizedDestinations.Keys)
            {
                var d = _categorizedDestinations[k];
                if (d[0].Category == category)
                    return d;
            }
            throw new Exception();
        }
