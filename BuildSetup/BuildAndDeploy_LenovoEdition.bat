...
IF %ERRORLEVEL% NEQ 0 GOTO EndBad
devenv /nologo S3DDriver-3rdParty.sln %BUILDTYPE% "Final Release - Lenovo|Win32" | bin\others\tee -a BuildSetup\build.log
IF %ERRORLEVEL% NEQ 0 GOTO EndBad
devenv /nologo S3DDriver-3rdParty.sln %BUILDTYPE% "Final Release - Lenovo|x64" | bin\others\tee -a BuildSetup\build.log
IF %ERRORLEVEL% NEQ 0 GOTO EndBad
...
call BuildSetup\LenovoEdition\_BuildSetup.bat
IF %ERRORLEVEL% NEQ 0 GOTO EndBad
CD BuildSetup

IF "%2"=="" call DeploySetup.bat LenovoEdition Lenovo

ECHO 

IF "%COMPUTERNAME%"=="BUILD-SERVER" (
	AddToSymbolServer.bat "final release"
	AddToSymbolServer.bat "final release - lenovo"
	ECHO 
)
