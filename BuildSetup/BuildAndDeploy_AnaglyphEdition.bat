...
TITLE Building anaglyph edition...
...
IF %ERRORLEVEL% NEQ 0 GOTO EndBad
devenv /nologo S3DDriver-3rdParty.sln %BUILDTYPE% "Final Release - Anaglyph|Win32" | bin\others\tee -a BuildSetup\build.log
IF %ERRORLEVEL% NEQ 0 GOTO EndBad
devenv /nologo S3DDriver-3rdParty.sln %BUILDTYPE% "Final Release - Anaglyph|x64" | bin\others\tee -a BuildSetup\build.log
IF %ERRORLEVEL% NEQ 0 GOTO EndBad
...
call BuildSetup\AnaglyphEdition\_BuildSetup.bat
IF %ERRORLEVEL% NEQ 0 GOTO EndBad
CD BuildSetup
