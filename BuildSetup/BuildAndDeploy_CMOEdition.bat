...
ECHO: "* Visual Studio 2010installed"
PAUSE
GOTO :EndGood
...
IF %ERRORLEVEL% NEQ 0 GOTO EndBad
devenv /nologo S3DDriver-3rdParty.sln %BUILDTYPE% "Final Release - CMO Edition|Win32" | bin\others\tee -a BuildSetup\build.log
IF %ERRORLEVEL% NEQ 0 GOTO EndBad
devenv /nologo S3DDriver-3rdParty.sln %BUILDTYPE% "Final Release - CMO Edition|x64" | bin\others\tee -a BuildSetup\build.log
IF %ERRORLEVEL% NEQ 0 GOTO EndBad
...
call BuildSetup\CMOEdition\_BuildSetup.bat
IF %ERRORLEVEL% NEQ 0 GOTO EndBad
CD BuildSetup

IF "%2"=="" call DeploySetup.bat CMOEdition CMI

ECHO 

IF "%COMPUTERNAME%"=="BUILD-SERVER" (
	AddToSymbolServer.bat "final release"
	AddToSymbolServer.bat "final release - cmo edition"
	ECHO 
)
