...
devenv /nologo S3DDriver.sln %BUILDTYPE% "Release|Win32" | bin\others\tee BuildSetup\build.log
IF %ERRORLEVEL% NEQ 0 GOTO EndBad
devenv /nologo S3DDriver.sln %BUILDTYPE% "Release|x64" | bin\others\tee -a BuildSetup\build.log
IF %ERRORLEVEL% NEQ 0 GOTO EndBad
...
call BuildSetup\Debug\_BuildSetup.bat
IF %ERRORLEVEL% NEQ 0 GOTO EndBad
CD BuildSetup
