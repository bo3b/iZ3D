...
IF %ERRORLEVEL% NEQ 0 GOTO EndBad
devenv /nologo S3DDriver-3rdParty.sln %BUILDTYPE% "Final Release - YGT|Win32" | bin\others\tee -a BuildSetup\build.log
IF %ERRORLEVEL% NEQ 0 GOTO EndBad
devenv /nologo S3DDriver-3rdParty.sln %BUILDTYPE% "Final Release - YGT|x64" | bin\others\tee -a BuildSetup\build.log
IF %ERRORLEVEL% NEQ 0 GOTO EndBad
...
call BuildSetup\YGTEdition\_BuildSetup.bat
IF %ERRORLEVEL% NEQ 0 GOTO EndBad
CD BuildSetup

IF "%2"=="" call DeploySetup.bat YGTEdition YGT

ECHO 

IF "%COMPUTERNAME%"=="BUILD-SERVER" (
	AddToSymbolServer.bat "final release"
	AddToSymbolServer.bat "final release - ygt"
	ECHO 
)
