...
namespace Commands
{
	...
	void CreateHullShader11_0::Execute( D3DDeviceWrapper *pWrapper )
	{
		BEFORE_EXECUTE(this);
		pWrapper->OriginalDeviceFuncs11.pfnCreateHullShader(pWrapper->hDevice, 
			pCode_, GET_SHADER_HANDLE(hShader_), hRTShader_, &Signatures_);
		AFTER_EXECUTE(this); 
	}

	bool CreateHullShader11_0::WriteToFile( D3DDeviceWrapper *pWrapper ) const
	{
		WriteStreamer::CmdBegin( pWrapper->GetWrapperHandle(), ( Command* )this,  "CreateHullShader11_0" );
	...
	bool CreateHullShader11_0::ReadFromFile()
	{
	...
	void CreateHullShader11_0::BuildCommand(CDumpState *ds)
	{
		fullCode = (char *)ds->LoadFromBinaryFile(databin_pos,&fullSize);

		output(glob,"ID3D11HullShader *hshader_%d;",EventID_);

		output(init,"char *shader_data_%d = GetFromFile2(%d);",EventID_,ds->ResaveBinaryData(databin_pos));
		output(init,"V_(device->CreateHullShader(shader_data_%d,%d,0,&hshader_%d));",EventID_,fullSize,EventID_);
									...
									D3D10DDI_HRTSHADER  hRTShader, CONST D3D11DDIARG_TESSELLATION_IO_SIGNATURES*  pSignatures)
{
	...
	pWrp->InitializeHS(D3D10_GET_WRAPPER(), pCode, pSignatures);
#ifndef EXECUTE_IMMEDIATELY_G2
	Commands::CreateHullShader11_0* command = new Commands::CreateHullShader11_0(pCode, hShader, hRTShader, pSignatures);
	D3D10_GET_WRAPPER()->AddCommand(command, true );
#else
	D3D10_GET_WRAPPER()->ProcessCB();
	{
		THREAD_GUARD_D3D10();
		D3D11_GET_ORIG().pfnCreateHullShader(D3D10_DEVICE, pCode, GET_SHADER_HANDLE(hShader), hRTShader, pSignatures);
	}
#endif
}
