...
namespace Commands
{
	...
	void HsSetShaderResources11_0::Execute( D3DDeviceWrapper *pWrapper )
	{
		for (UINT i = 0; i < NumValues_; i++)
		{
			if (Values_[i].pDrvPrivate)
			{
				...
				DEBUG_TRACE3(L"\t\t HS SRV %d: %s%d", i, pResWrp->GetResourceName(), resID);
#endif
		...
		BEFORE_EXECUTE(this);
		pWrapper->OriginalDeviceFuncs11.pfnHsSetShaderResources( pWrapper->hDevice,
			Offset_, 
			NumValues_, 
	...
	bool HsSetShaderResources11_0::WriteToFile( D3DDeviceWrapper *pWrapper ) const
	{
		return WriteToFileEx(pWrapper, "HsSetShaderResources11_0");
	}
}

VOID ( APIENTRY HsSetShaderResources11_0 )(
									   ...
									   )
{
	...
	Commands::HsSetShaderResources11_0* command = new Commands::HsSetShaderResources11_0( Offset, NumViews, phShaderResourceViews );
	...
	{
		THREAD_GUARD_D3D10();
		D3D11_GET_ORIG().pfnHsSetShaderResources( D3D10_DEVICE, Offset, NumViews, GetPointerToVector(SRVBuffer) );
	}
#endif	
}