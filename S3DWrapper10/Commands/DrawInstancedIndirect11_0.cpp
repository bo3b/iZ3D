...
namespace Commands
{
	...
	void DrawInstancedIndirect11_0::Execute( D3DDeviceWrapper *pWrapper )
	{
		BEFORE_EXECUTE(this);
		pWrapper->OriginalDeviceFuncs11.pfnDrawInstancedIndirect(pWrapper->hDevice, 
			GET_RESOURCE_HANDLE(hBufferForArgs_), AlignedByteOffsetForArgs_);
		AFTER_EXECUTE(this); 
	}

	bool DrawInstancedIndirect11_0::WriteToFile( D3DDeviceWrapper * pWrapper ) const
	{
		WriteStreamer::CmdBegin( pWrapper->GetWrapperHandle(), ( Command* )this,  "DrawInstancedIndirect11_0" );
	...
	bool DrawInstancedIndirect11_0::ReadFromFile()
	{
	...
	D3D10DDI_HRESOURCE	hBufferForArgs, UINT AlignedByteOffsetForArgs )
{
	...
	{
		THREAD_GUARD_D3D10();
		D3D11_GET_ORIG().pfnDrawInstancedIndirect(D3D10_DEVICE, GET_RESOURCE_HANDLE(hBufferForArgs), AlignedByteOffsetForArgs );
	}
#endif
}
