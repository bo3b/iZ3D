...
#include "Shaders_dx10/shaderHorizontalInterlaced.h"
#include "Shaders_dx10/shaderVerticalInterlaced.h"
#include "Shaders_dx10/shaderHorizontalInterlacedOptimized.h"
#include "Shaders_dx10/shaderVerticalInterlacedOptimized.h"

OUTPUT_API void* CALLBACK CreateOutputDX10( DWORD nMode_, DWORD nSpanMode_ )
{
	return new AvitridOutput( nMode_, nSpanMode_ );
}

AvitridOutput::AvitridOutput( DWORD nMode_, DWORD nSpanMode_ )
...
AvitridOutput::~AvitridOutput()
{
}

void AvitridOutput::InitializeResources( D3DDeviceWrapper* pD3DWrapper_ )
{
	...
	{
	case 1:
		ParseShaderBlob( g_shaderHorizontalInterlaced, sizeof( g_shaderHorizontalInterlaced ), &pCode );
		break;
	case 2:
		ParseShaderBlob( g_shaderVerticalInterlacedOptimized, sizeof( g_shaderVerticalInterlacedOptimized ), &pCode );
		break;
	case 3:
		ParseShaderBlob( g_shaderVerticalInterlaced, sizeof( g_shaderVerticalInterlaced ), &pCode );
		break;
	default:
		ParseShaderBlob( g_shaderHorizontalInterlacedOptimized, sizeof( g_shaderHorizontalInterlacedOptimized ), &pCode );
		break;
	}
...
void AvitridOutput::SetStates(  D3DDeviceWrapper* pDevice_, D3DSwapChain* pSwapChain_ )
{
...
void AvitridOutput::ClearResources( D3DDeviceWrapper* pD3DWrapper_ )
{
	D3D10DDI_DEVICEFUNCS& GET_ORIG = pD3DWrapper_->OriginalDeviceFuncs;
	D3D10DDI_HDEVICE hDevice = pD3DWrapper_->hDevice;
...
bool AvitridOutput::FindAdapter( IDXGIFactory* pDXGIFactory_ )
{
	...
	for ( UINT nAdapter = 0; pDXGIFactory_->EnumAdapters( nAdapter, &pDXGIAdapter ) != DXGI_ERROR_NOT_FOUND; ++nAdapter )
	{
		...
		for ( UINT nOutput = 0; pDXGIAdapter->EnumOutputs( nOutput, &pDXGIOutput ) != DXGI_ERROR_NOT_FOUND; ++nOutput )
		{
			...
			for( UINT j = 0; EnumDisplayDevices( OutputDesc.DeviceName, j, &MonitorDevice, 0x0 ); j++ )
			{
				...
				{
					wchar_t* pnpid = wcsstr( MonitorDevice.DeviceID, _T("ATD10") );
					if (pnpid)
					{
						int i = _wtoi (pnpid + 5);
						if (i >= 1 && i <= 30)
					...
					if (wcsstr( MonitorDevice.DeviceID, L"OST076E"))
						return true;
				}
			}
			pDXGIOutput->Release();
