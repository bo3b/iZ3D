...
public:
    wxMatrix2D() {}
    wxMatrix2D(const wxMatrix2D &matrix, bool full_copy = false) { Create(matrix, full_copy); }
    wxMatrix2D(int width, int height, bool zero = true)   { Create(width, height, zero); }
    wxMatrix2D(int width, int height, const double *data) { Create(width, height, data); }
    wxMatrix2D(int width, int height, double *data, bool static_data) { Create(width, height, data, static_data); }
