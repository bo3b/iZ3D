...
{
	size_t size = strlen(extName) + 2;
    char *padExtName = (char*)malloc(size);
	strcpy_s(padExtName, size, extName);
    strcat_s(padExtName, size, " ");
...
int glh_init_extensions(const char *origReqExts)
{
    ...
    if (NULL == sysExts) {
		...
		size_t size = sysExtsLen + winsysExtsLen + 3;
        sysExts = (char*)malloc(size);
        CHECK_MEMORY(sysExts);
        strcpy_s(sysExts, size, extensions);
        sysExts[sysExtsLen] = ' ';
        sysExts[sysExtsLen + 1] = 0;
        strcat_s(sysExts, size, winsys_extensions);
	...
	size_t size = reqExtsLen + 2;
    if (NULL == unsupportedExts) {
        unsupportedExts = (char*)malloc(size);
    } else if (reqExtsLen > strlen(unsupportedExts)) {
        unsupportedExts = (char*)realloc(unsupportedExts, size);
    }
        ...
        reqExt = (char*)EatNonWhiteSpace(reqExt))
    {
        ...
        *extEnd = (char)0;
        ...
        if (!ExtensionExists(reqExt, sysExts)) {    // don't try to get function pointers if on MacOS
            ...
            strcat_s(unsupportedExts, size, reqExt);
            strcat_s(unsupportedExts, size, " ");
            success = GL_FALSE;
        }
        *extEnd = saveChar;
    }
