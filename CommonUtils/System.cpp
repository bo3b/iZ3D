...
BOOL SetUserSecurityLevel(TCHAR* fileName)
{
	...
	if (ConvertStringSecurityDescriptorToSecurityDescriptor(
		L"O:AUD:(A;;GAFA;;;SY)(A;;GAFA;;;AU)",
		SDDL_REVISION_1, &psd, &cSDSize))
	{
		res = SetFileSecurity(fileName, DACL_SECURITY_INFORMATION, psd);
		LocalFree(psd);	
	}
...
void DeleteAllFiles( CONST TCHAR * dir )
{
	...
	HANDLE hFind;
	_stprintf_s(str, _T("%s\\*"), dir);
	hFind = FindFirstFile(str, &FindFileData);
	if (hFind != INVALID_HANDLE_VALUE) 
	{
		do
		{
			_stprintf_s(str, _T("%s\\%s"), dir, FindFileData.cFileName);
			DeleteFile(str);
...
{      
	...
	//hash = 0xD202EF8D;
	hash = startValue;
	register const BYTE *k = (const BYTE *)p;
	while (size--)
		hash = (hash >> 8) ^ crctab[(hash & 0xff) ^ *k++];
	return hash;
}

// using Intel optimized realization based on polynomial 0x1EDC6F41
DWORD CalculateCRC32CFast(const void* p_buf, UINT length, BYTE mode, DWORD running_crc)
{
	return crc32c_auto((uint32_t*)&running_crc, (const uint8_t*)p_buf, length, mode);
}
