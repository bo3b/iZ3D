#define BOOST_TEST_MODULE d3d9_resource_leak_tests
...
void CreateDirect3D(HWND& hWnd, LPDIRECT3D9& pD3D)
{
...
void CreateD3DDevice(LPDIRECT3D9 pD3D, HWND hWnd, D3DPRESENT_PARAMETERS& d3dpp, LPDIRECT3DDEVICE9& pd3dDevice)
{
	HRESULT hr = S_OK;
...
BOOST_AUTO_TEST_CASE( PrimarySwapChainRCCheck )
{
...
BOOST_AUTO_TEST_CASE( SetRenderTargetRCCheck )
{
	...
	IDirect3DSurface9* pSurf = NULL;
	hr = pd3dDevice->CreateRenderTarget(10, 10, D3DFMT_X8R8G8B8, D3DMULTISAMPLE_NONE, 0, FALSE,
		&pSurf, NULL);
	BOOST_REQUIRE( SUCCEEDED(hr) );
	UINT rc = 0;
...
BOOST_AUTO_TEST_CASE( SetDepthStencilRCCheck )
{
	...
	IDirect3DSurface9* pSurf = NULL;
	hr = pd3dDevice->CreateDepthStencilSurface(10, 10, D3DFMT_D24X8, D3DMULTISAMPLE_NONE, 0, FALSE,
		&pSurf, NULL);
	BOOST_REQUIRE( SUCCEEDED(hr) );
	UINT rc = 0;
	...
	hr = pd3dDevice->SetDepthStencilSurface(pSurf);
	BOOST_REQUIRE( SUCCEEDED(hr) );
