...
/************************************************************/
extern zipFile ZEXPORT zipOpen2 ( const TCHAR* pathname, int append, zipcharpc* globalcomment, zlib_filefunc_def* pzlib_filefunc_def )
{
    zip_internal ziinit;
    zip_internal* zi;
    int err=ZIP_OK;
...
extern zipFile ZEXPORT zipOpen ( const TCHAR* pathname, int append)
{
...
#    endif
    ...
    size_filename = (uInt)wcslen(filename);
    ...
    for (i=0;i<size_filename;i++)
        *(zi->ci.central_header+SIZECENTRALHEADER+i) = (char)*(filename+i);
